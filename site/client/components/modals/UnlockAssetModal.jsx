...
const VaultSection = ({
  ...
  console.log("Vault Section, authOptions:", authOptions);
  return (
    <>
        ...
        <div className="mt-0 w-full items-center justify-center text-center">
          ...
          <div className="w-full flex items-center justify-center">
            <Button
              ...
              onClick={handleClose}
            >
              <div className={buttonText != "Unlock" ? "mr-2" : ""}>
                {buttonText}
              </div>
              <ScaleLoader
                loading={buttonText != "Unlock"}
                color="#0ea5e9"
                height={20}
                margin={1}
                radius={7}
...
export function UnlockAssetModal({
  ...
  const completeUnlock = async () => {
    try {
      ...
      const customApiAuthOption = authOptions.filter(
        (a) => a.name === "Custom API MFA",
      );

      const customApiPayload = customApiAuthOption.length > 0 ? customApiAuthOption[0].otp : "";
